...
| `/email/receiveOne`   | POST   | Recibir un email por messageId          |
...
│   │   └── emailReceiveOne.ts   # Handler POST /email/receiveOne
│   ├── utils/
...
curl -X POST http://localhost:3001/email/receiveOne \
  -H "Content-Type: application/json" \
  -d '{
...
### Recepción exitosa (/email/receive)
...
### Recepción de un email por messageId (/email/receiveOne)
...
```json
{
  "status": "error",
  "email": [],
  "error": "Email with messageId '<invalid>' not found"
}
```
